...
void RCSwitch::transmit(HighLow pulses) {
  ...
  int delayHigh = this->protocol.pulseLength * pulses.high;
  int delayLow = this->protocol.pulseLength * pulses.low;
  
  if (delayHigh > 1950) {
    delay ( delayHigh / 1000 );
  } else {
...
bool RECEIVE_ATTR RCSwitch::receiveProtocol(const int p, unsigned int changeCount) {
    ...
    unsigned long long alternatecode = 0;
    ...
    unsigned long long variancebitduration = (squareddataduration - (unsigned long)dataduration*(unsigned long)dataduration/numberofdatabits)/(numberofdatabits-1);
    unsigned long long alternatevariancebitduration = (alternatesquareddataduration - (unsigned long)alternatedataduration*(unsigned long)alternatedataduration/numberofdatabits)/(numberofdatabits-1);
    ...
    unsigned long long squaredaveragebitduration = 0;
